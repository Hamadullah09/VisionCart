 ...
 *
 *   public/wasm/*                    copied from node_modules (offline)
 *   public/models/face_landmarker.task   downloaded once from Google's CDN
 *
...
const modelDest = path.join(root, "public", "models", "face_landmarker.task");
...
async function fetchModel() {
  ...
  try {
    ...
    console.log(`✓ saved public/models/face_landmarker.task (${(buf.length / 1e6).toFixed(1)} MB)`);
    return true;
  } catch (err) {
    ...
    console.warn(`  Or download manually to public/models/face_landmarker.task from:\n  ${MODEL_URL}\n`);
    return false;
  }
}
